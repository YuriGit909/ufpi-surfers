...
void createObstacle(float z)
{
    ...
    if (adaptType == BUS)
    {
        ...
        else if (pattern == 1)
        {
            int free = rand() % 3;
            for (int i = 0; i < 3; i++)
                if (i != free) createBusAt(LANES[i], z, false);
        }
        else
        {
    ...
    else if (adaptType == SPEED_BUMP)
    {
        ...
        else if (pattern == 1)
        {
            int free = rand() % 3;
            for (int i = 0; i < 3; i++)
                if (i != free) createObstacleAt(LANES[i], z, SPEED_BUMP);
        }
        else
        {
            for (int i = 0; i < 3; i++)
    ...
    else
    {
        ...
        else if (pattern == 1)
        {
            int free = rand() % 3;
            for (int i = 0; i < 3; i++)
                if (i != free) createObstacleAt(LANES[i], z, LOW_BARRIER);
        }
        else
            for (int i = 0; i < 3; i++)
...
void updateObstacles(float speed, float score)
{
    ...
    for (auto &obs : obstacles)
    {
        ...
        if (obs.z > 15.0f && !obs.isChaser)
        {
            ...
            {
                obs.type = SPEED_BUMP;
                if (rand() % 100 < 40) spawnCoinArc(obs.x, obs.z);
            }
            else
            {
                obs.type = LOW_BARRIER;
                if (rand() % 100 < 70) spawnCoinArcLow(obs.x, obs.z);
            }
        }
...
bool canMoveToLane(float targetX)
{
    ...
    for (auto &obs : obstacles)
    {
        ...
        if (obs.type == BUS)
        {
            ...
            {
                if (sideHitWarning) gameOver = true;
                else { sideHitWarning = true; sideHitTimer = 0; }
                return false;
            }
        }
    }
